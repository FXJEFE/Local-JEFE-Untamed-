...
class EnhancedAgent:
    ...
    def __init__(self, working_dir: str = None):
        ...
        self.working_dir = os.path.abspath(
            working_dir or LARRY_CONFIG.get("working_directory") or str(BASE_DIR))
        ...
        if PRODUCTION_RAG_AVAILABLE:
            try:
                ...
                logger.info(
                    f"Production RAG initialized. Stats: {self.rag.get_stats()}")
            except Exception as e:
        ...
        if MCP_AVAILABLE:
            self.mcp = get_mcp_toolkit(
                os.path.join(self.working_dir, "mcp.json"))
        else:
            self.mcp = None
        ...
        try:
            ...
            if recent:
                ...
                # Deeper integration: semantic retrieval using embeddings
                if self.embeddings and self.embeddings.vectorstore:
                    try:
                        query = "recent important context and tasks from previous agent sessions"
                        docs = self.embeddings.similarity_search(query, k=3)
                        if docs:
                            semantic_context = "\n\n".join([d.page_content[:400] for d in docs])
                            self.system_prompt += f"\n\n[SEMANTIC MEMORY RETRIEVAL]\n{semantic_context}"
                            logger.info("Injected semantic memory from vector store")
                    except Exception as e:
...
Answer concisely and technically, citing file names and line numbers when possible:"""
    ...
    def process_query_multi(self, query: str, history: list = None,
        ...
        response = self.router.generate(
            full_prompt, model=model, options=options)
            ...
            if user_input.startswith("/"):
                ...
                # ── FXJEFE Local MCP Tools (for testing tool use with Ollama) ──
                    ...
                    if not (MCP_AVAILABLE and hasattr(agent.mcp, 'fxjefe') and agent.mcp.fxjefe and agent.mcp.fxjefe.available):
                        print("FXJEFE Local MCP server not available. Run: python mcp/fxjefe-local-mcp/fxjefe_local_mcp_server.py")
                        continue
                    if not args:
                    ...
                    if MCP_AVAILABLE and agent.mcp:
                        ...
                        if hasattr(agent.mcp, 'fxjefe') and agent.mcp.fxjefe:
                            ...
                            if fx.available:
                                ...
                                print("  Launch: python mcp/fxjefe-local-mcp/fxjefe_local_mcp_server.py")
                            print()
                        ...
                        try:
                            ...
                            loaded = []
                        print(f" In-process servers loaded: {len(loaded)}")
                        if loaded:
                            print(f"   {', '.join(loaded)}")
                        print()

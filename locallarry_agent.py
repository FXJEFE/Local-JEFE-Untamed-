#!/usr/bin/env python3
"""
OpenClaws Agent v2.0 - The Ultimate Self-Improving Proactive Ollama Agent
...
# ============== CONFIG ==============
MODEL = os.getenv("OPENCLAWS_MODEL", "llama3.1:8b")  # or qwen2.5-coder, deepseek-r1, etc.
MEMORY_FILE = "openclaws_memory.json"
KG_FILE = "openclaws_knowledge_graph.graphml"
LOGS_DIR = Path("openclaws_logs")
...
class TypedKnowledgeGraph:
    def __init__(self, filepath: str = KG_FILE):
        ...
        self.valid_types = {"Person", "Project", "Task", "Event", "Document", "Skill", "Memory", "Issue", "PR", "Video"}
    ...
    def link(self, source_id: str, target_id: str, relation: str, weight: float = 1.0):
        self.graph.add_edge(source_id, target_id, relation=relation, weight=weight, timestamp=datetime.now().isoformat())
        self._save()
    ...
    def plan_multi_step(self, goal: str) -> List[str]:
        ...
        related = self.query(name_contains=goal.split()[0] if goal else "", limit=5)
        for i, r in enumerate(related, 2):
            plan.append(f"{i}. Leverage existing {r.get('type', 'entity')}: {r['id']}")
...
Be honest and specific. Then give the improved version or patch."""
    ...
    def apply_self_patch(self, patch_description: str) -> bool:
        ...
        patch_file.write_text(f"# OpenClaws Self-Improvement Patch\n\n{patch_description}\n\nApply manually or ask agent to edit.")
        print(f"[OpenClaws] Self-improvement patch saved to {patch_file}")
        return True
...
{{"score": 0-100, "issues": ["list"], "improvements": ["list"], "approved": true/false, "verdict": "short human sentence"}}"""
    try:
        resp = ollama.chat(model=model, messages=[{"role": "user", "content": prompt}], options={"temperature": 0.2})
...
class OpenClawsAgent:
    def __init__(self, model: str = MODEL):
        ...
        print(humanize("OpenClaws online. We are way better than the competition. Let's dominate."))
    ...
    def _load_memory(self):
        if os.path.exists(MEMORY_FILE):
            ...
            self.memory = {"interactions": [], "skills_vetted": [], "created": datetime.now().isoformat()}
    ...
    def run(self, user_input: str) -> str:
        ...
        print(humanize(f"Quick plan I just made: {plan[:200]}..."))
        ...
        print(humanize(f"Done in {elapsed:.1f}s. Memory updated. Knowledge graph now has {self.kg.graph.number_of_nodes()} nodes."))
    ...
    def _get_relevant_context(self, query: str) -> str:
        ...
        kg_hits = self.kg.query(name_contains=query.split()[0] if query else "", limit=3)
...
Respond in natural human bullet points."""
    ...
    def _spontaneous_self_improvement(self):
        ...
        print(humanize("Quick self-check... vetting one of my own skills for fun."))
        ...
        if not vet.get("approved", True):
            self.improver.apply_self_patch(f"Improve github_skill based on vet: {vet['verdict']}")
    ...
    def github_skill(self, query: str) -> str:
        ...
        try:
            ...
            if "issue" in query.lower():
                ...
                cmd = ["gh"] + query.split()[1:] if query.startswith("gh ") else ["gh", "api", "user"]
    ...
    def docx_skill(self, action: str) -> str:
        ...
        try:
            if "create" in action.lower() or "new" in action.lower():
                ...
                doc.add_paragraph(humanize("This doc was created by the superior OpenClaws agent."))
                filename = f"openclaws_{int(time.time())}.docx"
                doc.save(filename)
                self.kg.add_entity("Document", filename, {"path": os.path.abspath(filename)})
    ...
    def obsidian_skill(self, action: str) -> str:
        ...
        try:
            if "search" in action.lower():
                ...
                self.kg.add_entity("Document", title, {"vault_path": str(path)})
    ...
    def youtube_watcher_skill(self, query: str) -> str:
        ...
        try:
            # Extract video ID
            vid_match = re.search(r'(?:v=|youtu.be/)([a-zA-Z0-9_-]{11})', query)
            if not vid_match:
                return "Give me a YouTube URL or video ID"
            vid = vid_match.group(1)
            transcript = YouTubeTranscriptApi.get_transcript(vid)
            full_text = " ".join([t['text'] for t in transcript])
            summary = self._ollama(f"Summarize this YouTube transcript in natural human style:\n{full_text[:8000]}", temperature=0.5)
            self.kg.add_entity("Video", f"yt_{vid}", {"transcript_length": len(full_text), "summary": summary[:200]})
    ...
    def find_skills_skill(self, query: str) -> str:
        ...
        # LLM discovery
        discovered = self._ollama(f"User wants skills about: {query}. Suggest 5 powerful new skills OpenClaws should add (name + one-sentence description).", temperature=0.8)
    ...
    def mcporter_skill(self, cmd: str) -> str:
        ...
        try:
            result = subprocess.run([MCPORTER_PATH] + cmd.split(), capture_output=True, text=True, timeout=45)
    ...
    def session_logs_skill(self, query: str = "") -> str:
        ...
        try:
            with open(latest) as f:
                data = json.load(f) if latest.suffix == ".json" else {"raw": f.read()[:500]}
...
if __name__ == "__main__":
    print("=" * 60)
    print(humanize("OPENCLAWS AGENT v2.0 - SELF-IMPROVING PROACTIVE SYSTEM LOADED"))
    ...
    while True:
        try:
            ...
            if not user or user.lower() in ["exit", "quit", "bye"]:
                print(humanize("Session saved. Knowledge graph updated. See you soon."))
                break
            response = agent.run(user)
            print(f"\nOpenClaws: {response}")
        except KeyboardInterrupt:
            print(humanize("\nCaught interrupt. Self-saving everything..."))
            break
        except Exception as e:
            print(f"Critical error (self-reflecting...): {e}")
            agent.improver.reflect_and_critique("main loop crash", str(e), False)
